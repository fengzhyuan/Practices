...
namespace KDTree
{
  ...
  template <size_t const __K, typename _Val, typename _SubVal,
    ...
    struct _M_Search_Region
    {
      ...
      // Note the region (first) component is not supposed to have an area, its
      // bounds should all be set to a specific point.
      typedef std::pair<_M_Search_Region,_SubVal> _CenterPt;

      _M_Search_Region(_Acc const& __acc=_Acc(), const _Cmp& __cmp=_Cmp())
	: _M_cmp(__cmp), _M_acc(__acc) {}

      template <typename Val>
      _M_Search_Region(Val const& __V,
	      _Acc const& __acc=_Acc(), const _Cmp& __cmp=_Cmp())
	: _M_acc(__acc), _M_cmp(__cmp)
      {
        for (size_t __i = 0; __i != __K; ++__i)
      ...
      /***
       * modification of search range; 
       * from __R (arbitrary dim) to specified region defined
       * by the two points [__Vlo, __Vhi]
       */
      template <typename Val>
	...
	: _M_acc(__acc), _M_cmp(__cmp)
      ...
      bool
      intersects_with(_M_Search_Region const& __THAT) const
      {
        for (size_t __i = 0; __i != __K; ++__i)
          {
            if (_M_cmp(__THAT._M_high_bounds[__i], _M_low_bounds[__i])
